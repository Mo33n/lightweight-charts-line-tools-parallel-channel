...
import { HitTestResult, HitTestType, PaneCursorType, lineThroughPoints, Point, clipPolygonToViewport, extendAndClipLineSegment, setLineStyle, drawLine, pointInPolygon, distanceToSegment, LineToolPaneView, SegmentRenderer, LineCap, LineJoin, AnchorPoint, BaseLineTool, deepCopy, merge, interpolateLogicalIndexFromTime, interpolateTimeFromLogicalIndex, InteractionPhase, getToolCullingState, OffScreenState } from 'lightweight-charts-line-tools-core';
...
class ParallelChannelRenderer {
    ...
    draw(target) {
        ...
        target.useMediaCoordinateSpace(({ context: ctx, mediaSize }) => {
            ...
            if (lineTop.b !== 0) {
                if (extend.left) {
                    p_start_top = new Point(X_FAR_LEFT, (m_top * X_FAR_LEFT + b_top));
                    p_start_bottom = new Point(X_FAR_LEFT, (m_bottom * X_FAR_LEFT + b_bottom));
                }
                if (extend.right) {
                    p_end_top = new Point(X_FAR_RIGHT, (m_top * X_FAR_RIGHT + b_top));
                    p_end_bottom = new Point(X_FAR_RIGHT, (m_bottom * X_FAR_RIGHT + b_bottom));
                }
            }
    ...
    hitTest(x, y) {
        ...
        const checkLine = (pA, pB, cursor) => {
            ...
            if (segmentOrPoint === null || segmentOrPoint instanceof Point)
                return null;
    ...
    _drawLine(ctx, pA, pB, options, extend, W, H) {
        const segmentOrPoint = extendAndClipLineSegment(pA, pB, W, H, extend.left, extend.right);
        if (segmentOrPoint instanceof Point)
            return;
 ...
 */
class LineToolParallelChannelPaneView extends LineToolPaneView {
    /**
    ...
    _addAnchors(renderer) {
        ...
        const getScreenPoint = (index) => {
            ...
            const screenPoint = logicalPoint ? tool.pointToScreenPoint(logicalPoint) : new Point(0, 0);
 ...
 */
class LineToolParallelChannel extends BaseLineTool {
    /**
    ...
    constructor(coreApi, chart, series, horzScaleBehavior, options = {}, points = [], priceAxisLabelStackingManager) {
        ...
        const finalOptions = deepCopy(ParallelChannelOptionDefaults);
    ...
    getPoint(index) {
        ...
        const idxA = interpolateLogicalIndexFromTime(this._chart, this.getSeries(), midPointA.timestamp);
        const idxB = interpolateLogicalIndexFromTime(this._chart, this.getSeries(), midPointB.timestamp);
    ...
    updateCullingState() {
        ...
        const cullingState = getToolCullingState(channelCorners, this, options.extend, undefined, undefined, true // isAreaBased: true
        );
...
export { LineToolParallelChannel, registerParallelChannelPlugin as default, registerParallelChannelPlugin };
//# sourceMappingURL=lightweight-charts-line-tools-parallel-channel.js.map
